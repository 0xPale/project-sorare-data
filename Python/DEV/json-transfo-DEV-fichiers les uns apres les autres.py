...
#DEV with only a few files
read_files = glob.glob(output_folder + "dev/*.json")

df_card_concat = pd.DataFrame()
df_player_concat = pd.DataFrame()
df_transfer_concat = pd.DataFrame()
df_cardSupply_concat = pd.DataFrame()
df_allSo5Scores_concat = pd.DataFrame()
...
#On drop les duplicate qui peuvent apparaître dans les data liées aux players (car elles vont être dupliquées pour chaque card de ce player)
        ...
        df_card_concat = pd.concat([df_card_concat, df_card])
        df_player_concat = pd.concat([df_player_concat, df_player])
        df_transfer_concat = pd.concat([df_transfer_concat, df_transfer])
        df_cardSupply_concat = pd.concat([df_cardSupply_concat, df_cardSupply])
        df_allSo5Scores_concat = pd.concat([df_allSo5Scores_concat, df_allSo5Scores])

#Export csv
df_card_concat.to_csv(output_folder + "csv/card.csv", sep=";", index= False)
df_player_concat.to_csv(output_folder + "csv/player.csv", sep=";", index= False)
df_transfer_concat.to_csv(output_folder + "csv/transfer.csv", sep=";", index= False)
df_cardSupply_concat.to_csv(output_folder + "csv/cardSupply.csv", sep=";", index= False)
df_allSo5Scores_concat.to_csv(output_folder + "csv/allSo5Scores.csv", sep=";", index= False)
